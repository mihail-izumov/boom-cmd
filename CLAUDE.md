...
(`Все парки / MARI / Охта Молл / Питерленд / Июнь`):
...
выбор делается **центрированным чёрным беджем** под центрированным
заголовком (сейчас — только на Проектах, флаг `parkFilter: true` у вкладки).
Тап по беджу открывает bottom-sheet chooser (`ParkPickerSheet.vue`).
Имена парков — единый справочник `src/data/parks.js`. Аналитика/Материалы —
заглушки, парк-контекст к ним не подключён.
...
- **Шапка навигации — центрированный статичный заголовок** (`h-14` + safe-area),
  без iOS-collapse large-title. Слева — back-кнопка при заходе вглубь;
  под заголовком — слот `belowTitle` (сейчас рендерит чёрный бедж
  парк-фильтра на Проектах).
...
## Структура кода (актуально для Фазы 3, ревизия 3.1)
- `src/App.vue` — конфиг 4 вкладок (Главная стартовая) + конфиг под-страниц
  (сейчас `parks`). Флаг `parkFilter: true` у вкладки → бедж активного
  парк-фильтра в шапке.
- `src/components/AppShell.vue` · `NavigationBar.vue` · `TabBar.vue` — оболочка.
  Шапка центрированная, без collapse; навбарного селектора-иконки больше нет.
  `NavigationBar` имеет scoped slot `belowTitle` (используется для бедж-фильтра).
- `src/components/navigation/ParkFilterBadge.vue` — центрированный чёрный
  бедж активного парк-фильтра (тап → chooser).
- `src/components/navigation/ParkPickerSheet.vue` — bottom-sheet chooser
  (выбор парка: «Все парки» + группы Москва/СПб).
- `src/components/home/SectionBanner.vue` — крупный баннер деки на Главной
  (иконка + заголовок раздела + плейсхолдер цифр; тап → вкладка).
