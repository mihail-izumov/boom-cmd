...
export function verdict(d = {}) {
  ...
  // ⚠ ОБРЫВ ПРОБЫ ПО ПОТОЛКУ — НЕ ТО ЖЕ, ЧТО ОТКАЗ ИСТОЧНИКА (13.08.2026).
  // Потолок пробы — 2 с (PROBE_TIMEOUT_MS), а `doGet` Apps Script отрабатывает
  // 5–11 с по журналу выполнений, на холодном старте дольше. То есть проба,
  // упёршаяся в потолок, — рядовое явление, а не признак блокировки. Найдено на
  // ПЕРВОЙ ЖЕ живой заявке: `probe_api_ms = 2004` при потолке 2000, и вердикт
  // объявил «Google недоступен — VPN или сеть ТЦ». Оставь мы так — каждое
  // расследование начиналось бы с поисков VPN, которого может не быть вовсе.
  // Различает ветки `probe_api_error`: 'timeout' ставит только AbortController.
  if (selfOk && !apiOk && d.probe_api_error === 'timeout') {
    return {
      code: 'api-slow',
      title: 'Источник не ответил за 2 секунды',
      advice: 'Это ещё не поломка: источник просыпается медленно. Нажмите СТАРТ ещё раз — вход ждёт дольше, чем эта проверка.',
    }
  }
  if (selfOk && !apiOk) {
    return {
