...
// Сортировка по вкладу вниз; ширина мини-полосы — доля от МАКСИМАЛЬНОГО вклада
// (сравнение драйверов между собой), не от ёмкости.
export function itemsFor(data, park) {
  ...
  return sorted.map((r) => ({
    ...
    pct_in: num(r.pct_in),
    bg: r.bg === true,
    barPct: clamp((Math.abs(pos(r.rub)) / max) * 100),
  }))
}

// ── Строки карточки «Парки — закрытие ёмкости» ──────────────────────────────
