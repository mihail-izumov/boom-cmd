...
//
// ── ОФЛАЙН НЕ ПОВТОРЯЕМ (12.08.2026, вариант A) ──
//
// ПОВОД. Владелец проверял гейт в Safari без связи: повторы отработали, но
// «очень медленно». Разбор по секундомеру: `fetch` без сети падает мгновенно,
// поэтому все 5.5 секунды — это ЧИСТЫЕ ПАУЗЫ между попытками (1500 + 4000).
// Человек ждал ровно то время, которое политика отводит на надежду, что сеть
// оправится, — при том что браузер с самого начала знал, что сети нет.
//
// ПОЧЕМУ ЭТО НЕ ПРОТИВОРЕЧИТ САМОЙ ПОЛИТИКЕ. Повторы лечат ОСЕЧКУ: запрос ушёл
// и не дошёл (05.08, сеть ТЦ). Отсутствие интернета — не осечка, а состояние:
// вторая и третья попытки заведомо повторят первую с тем же исходом.
//
// ГРАНИЦА ДОВЕРИЯ К `navigator.onLine`. Он врёт ТОЛЬКО в одну сторону —
// говорит «связь есть» при мёртвом Wi-Fi или капчапортале (тогда всё идёт
// прежним путём, с повторами, ничего не теряем). Обратное враньё — «связи
// нет», когда она есть — на практике не встречается: флаг снимается по
// исчезновению сетевого интерфейса, а не по качеству канала. Поэтому короткое
// замыкание здесь безопасно, а в обратную сторону (пускать без проверки, раз
// `onLine === true`) — нет, и мы этого не делаем.
//
// ЧТО НЕ СДЕЛАНО СОЗНАТЕЛЬНО: понижение потолка ожидания попытки (25 с) — это
// вариант B, отложен в `docs/BACKLOG-boom-cmd.md` §12 до накопления реальных
// `ms`/`attempts` из заявок «Проблемы со входом».
...
async function submitKey(phrase) {
  ...
  // Короткое замыкание на офлайне — ДО `checking`: показывать «Проверяем…»
  // ради заведомо невозможной проверки значит врать о том, что происходит.
  // `attempts: 0` и `ms: 0` в диагностике честны буквально: попыток не было,
  // время не потрачено; чем закончилось — говорит `kind`, а не пустое поле.
  if (!isOnline()) {
    netError.value = 'Нет связи с источником данных'
    netHint.value = networkHint({ retriable: true, online: false })
    lastFailure.value = {
      kind: 'offline',
      message: 'Браузер сообщает: сети нет — запрос не отправлялся',
      http: '',
      attempts: 0,
      ms: 0,
    }
