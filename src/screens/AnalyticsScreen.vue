...
import { PERIODS, monthLabel, monthShortCap, updatedDateLabel } from '../i18n/analytics.js'
...
// Подпись над заголовком — ОДНА строка: «обновлено DD.MM.YYYY», где дата —
// РЕАЛЬНАЯ дата последнего изменения источника (поле `updated` из ответа
// Apps Script = getLastUpdated Google-таблицы). Ручная константа DATA_AS_OF
// убрана; дату «сегодня» НЕ подставляем (старый баг). Если `updated` нет или
// не парсится — подпись пустая (лучше ничего, чем ложная дата).
const updatedLabel = computed(() => {
  const upd = updatedDateLabel(data.value?.updated)
  return upd ? `обновлено ${upd}` : ''
})
