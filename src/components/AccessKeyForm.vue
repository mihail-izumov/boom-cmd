    ...
    <div class="flex flex-1 items-start justify-center pt-[13svh]">
      ...
      >
        ...
        <!-- Бейдж уровня продукта (D-23, 19.08.2026). Третий ярус лочкапа, а не
             ...
             ОДНОГО ЗНАЧЕНИЯ В EM НЕ ХВАТИЛО. Расчёт «сдвиг пропорционален кеглю»
             верен только внутри одного движка: где именно движок ставит базовую
             линию в строке, зависит от того, какие метрики шрифта он читает
             (hhea / OS/2 typo / win), а у этого файла они расходятся — hhea 800/200
             против winAscent 969. Замер на десктопе (то же место, 2 px на CSS-px:
             рамка 471…534, капс 490…519) показал зазоры 19 сверху и 15 снизу,
             то есть мобильные 0.095em там ПЕРЕЛЕТАЮТ на 1 CSS-px. Отсюда
             отдельное md:top-[0.05em] — не «на всякий случай», а по замеру.
             relative/top, а не margin: правка чисто оптическая, высоту строки и
             раскладку она трогать не должна.
             ТРЕКИНГ 0.32em (решение владельца 19.08, было 0.16em): слово должно
             занимать рамку, а не сидеть в ней с воздухом по краям. Ширину бейджа
             это почти не меняет (104 → 108px) — меняется плотность, потому что
             поля сокращены синхронно с разгонкой: 15 → 10px. Дальше не пошли:
             на 0.48em «УЛЬТРА» распадается на шесть отдельных знаков и перестаёт
             читаться с одного взгляда, а бейдж — не декор.
             ПОЛЯ ЗАДАНЫ ПРОПОРЦИЕЙ ОТ КЕГЛЯ, а не двумя независимыми числами:
             10/17.5 = 12/21 = 0.571em. Поэтому на десктопе бейдж занимает от
             слова ровно ту же долю, что на мобайле (61%), и связка выглядит
             одинаково на обоих брейкпоинтах.
             mr компенсирует трекинг после последней буквы, как у слова выше, и
             обязан идти с ним в паре: разгонка 0.32em → компенсация -0.32em,
             иначе слово уедет влево на пол-интервала.
        ...
        <span
          ...
          class="mt-0 flex h-[27px] items-center justify-center border-[1.5px] border-[var(--text)] px-[10px] font-brand text-[1.09375rem] uppercase leading-none tracking-[0.32em] text-[var(--text)] md:h-[32px] md:border-2 md:px-[12px] md:text-[1.3125rem]"
          aria-hidden="true"
        ><span class="relative top-[0.095em] mr-[-0.32em] md:top-[0.05em]">Ультра</span></span>
      </div>
    </div>
