...
<template>
  ...
  >
    <!-- ГРУППА «логотип + карточка» (правка 29.07).
         Раньше экран был собран из двух эластичных зон: лого сверху с отступом
         13% высоты, карточка посередине, подвал снизу. На коротком мокапе (750px)
         это смотрелось, а на высоком телефоне зоны растягивались и лого улетало
         от карточки на треть экрана — изолированный в пустоте знак читается мельче,
         чем он есть, хотя пропорция к карточке (44% ширины) в норме.
         Теперь лого и карточка — ОДИН блок с фиксированным зазором 40px, блок
         центрируется целиком, подвал прижат отдельно. Так пропорция держится
         на любой высоте экрана: от SE до Pro Max меняется только воздух вокруг. -->
    <div data-test="access-group" class="flex flex-1 flex-col items-center justify-center gap-10 py-8 md:gap-12">
      <div
      ...
      <!-- карта ввода — второй элемент той же группы, а не отдельная зона -->
      <form class="w-full max-w-[20rem]" @submit.prevent="onSubmit">
    ...
    <!-- футер: только логотип «Модуль роста» под маску, приглушённым --graphite.
         ...
         29.07: подвал больше НЕ эластичная зона (был flex-1) — он занимает свою
         высоту и прижат к низу, а весь свободный воздух отдан группе выше. -->
    <div class="flex shrink-0 flex-col items-center pb-10">
