...
// «осталось 2 дня» и «осталось 5 дней». Форма глагола берётся тем же pluralRu и с
// того же числа — иначе на единице бейдж читается как ошибка («Осталось 1 день»).
// 21, 31 и т.д. попадают в первую форму («остался 21 день») — это верно;
// 11 в неё не попадает (правило mod100 !== 11), «осталось 11 дней».
const daysWord = (n) =>
