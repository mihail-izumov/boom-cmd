...
// текст `--ink-on-color`.
//
// ВАЖНО (фикс бага «прозрачные тёмные карточки»):
//   Класс `bg-[var(--text)]` и инлайн `:style="{ '--text': … }"` на одном
//   элементе конфликтуют: фон вычисляется уже с подменённым токеном →
//   #FFFFFF, и карточка оказывается белой. Решение — два уровня:
//     • <article> держит фон/цвет (видит РОДИТЕЛЬСКИЕ значения токенов);
//     • вложенный <div> держит override CSS-токенов для ДЕТЕЙ.
//   Тогда children видят светлые `--text/--text-muted/--text-secondary/
//   --line/--surface-2`, а сам фон карточки остаётся тёмным.
//
...
const props = defineProps({
  ...
  emphasis: { type: Boolean, default: false }, // тёмная карточка слоя 1
})
...
const innerStyle = computed(() => (props.emphasis ? darkVars : null))
</script>
...
<template>
  ...
  >
    <!-- inner wrapper — здесь живут CSS-var overrides; на сам <article>
         их ставить нельзя, иначе `bg-[var(--text)]` посчитается с
         перекрашенным токеном и фон станет белым (это и был баг). -->
