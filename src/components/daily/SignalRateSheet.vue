...
//
// КОНТУР Б: отказаться от оценки можно — крестик и тап по фону записывают прочтение
// (это делает родитель), отметка не становится заложником оценки. Ноль при этом не
// отправляется: поля score в теле запроса просто нет, строка в signal_scores не
// появляется.
//
// ⚠ Отдельной кнопки «Отметить без оценки» здесь НЕ ДЕРЖИМ (решение владельца 15.08).
// Функционально она дублировала крестик, а выглядела как законная дверь мимо оценки —
// заметное предложение пропустить ровно тот шаг, ради которого весь релиз. Чинить
// потерю оценок и тут же приглашать её пропустить — противоречие; выход остаётся, но
// он не конкурирует с «Отправить».
