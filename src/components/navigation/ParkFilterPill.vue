...
//
// `compact` (TZ-3.2 §4):
//   false — полное имя парка / «Все парки» + рамка/фон.
...
const { current, isAll } = useParkContext()

const fullLabel = computed(() =>
  isAll.value ? 'Все парки' : PARKS_BY_ID[current.value]?.name || 'Все парки',
)
const shortLabel = computed(() => {
  if (isAll.value) return 'Все парки'
  const p = PARKS_BY_ID[current.value]
  return p?.short || p?.name || 'Все парки'
})
