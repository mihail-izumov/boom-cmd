...
//   2) «Вклад драйверов» — 100 % шкалы = ёмкость драйверов, закрытие ёмкости;
//   3) «Из чего складывается» — строка на драйвер (ТОЛЬКО на экране парка).
// «Вся сеть»: 1 и 2 на суммах + «Парки — закрытие ёмкости»; ДЕТАЛИЗАЦИИ НЕТ,
...
<template>
  <div class="flex flex-col gap-2.5">
    ...
    <template v-if="ready">
      ...
      <!-- ── 3b. Парки — закрытие ёмкости (только «Вся сеть») ───────────── -->
      <section
        v-if="!park"
        data-test="parks"
        class="bc-fade-in rounded-2xl bg-[var(--surface)] p-4 shadow-[var(--card-shadow)]"
