...
const subViews = {
  ...
  // `parkFilter: true`: парк выбирается ГЛОБАЛЬНОЙ выпадающей пилюлей в шапке
  // (как «Задачи»/«Материалы»). Внутри статус — слайдером, список — свёрнутыми
  // группами. Незапущенные драйверы видны при любом выборе парка (логика в
  // driversModel.matches — «Вся сеть» и парк трактуются как в Проектах).
  drivers: {
